...
use crate::utils::{get_multiple_accounts, get_account, get_amm_keys, get_keys_for_market};
use crate::types::{
    AmmInfo, AmmKeys, CalculateResult, MarketPubkeys};
...
impl PoolLoader {
    ...
    pub fn calc_pool_valut_amounts(
        ...
        let [_, amm_target_account, amm_pc_vault_account, amm_coin_vault_account, amm_open_orders_account, market_account, market_event_q_account] = accounts;
        ...
        let amm_pc_vault = Account::unpack(&amm_pc_vault_account.as_ref().unwrap().clone().data).unwrap();
    ...
    pub fn load_state(
        ...
        let amm_keys: AmmKeys = get_amm_keys(&amm_program_key, &amm_pool_key, &amm_info).unwrap();
